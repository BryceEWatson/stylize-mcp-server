...
class DeviceFingerprintService:
    ...
    def extract_timezone_offset(self, request: Request) -> int | None:
    ...
    def detect_fingerprint_spoofing(self, current: DeviceFingerprint,
        ...
        if len(same_ip_fingerprints) > 0:
            unique_fingerprints = len({h.device_fingerprint for h in same_ip_fingerprints})
        ...
        unique_ips = len({h.ip_address for h in same_fingerprint_ips})
...
def extract_fingerprint_from_request(request: Request,
                                   client_data: dict | None = None) -> DeviceFingerprint:
