...
def get_services():
    ...
    try:
        from app.main import get_style_service, get_openai_service, get_gcs_service, get_context_analysis_service, get_auth_service
        return {
...
async def authenticate_mcp_request(request_data: Dict[str, Any]) -> Optional[Dict]:
...
@mcp.tool()
async def list_styles(api_key: Optional[str] = None) -> List[Dict[str, str]]:
...
@mcp.tool()
async def get_style_details(style_id: str, api_key: Optional[str] = None) -> Dict[str, Any]:
...
async def styles_catalog(api_key: Optional[str] = None) -> str:
